...
class MLP():
    ...
    def _init_params(self):
        ...
        return [self._random_layer_params(
            self.size, self.size, k) for k in keys]
...
class GraphNetBlock():
    ...
    def __init__(self, cfg, name='GraphNetBlock'):
        self.name = name
        # Manual seeding. Is there a better way of doing this?
        self._edge_mlp = MLP(cfg, seed=0, name='edge_mlp')
        self._node_mlp = MLP(cfg, seed=1, name='node_mlp')

    def _update_edge_features(
            self, node_features, edge_features, senders, receivers):
