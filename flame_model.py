...
class Model():
    ...
    def __init__(self, name='Model'):
        ...
        self._output_normalizer = normalization.Normalizer(
            size=1, name='output_normalizer')  # Output is scalar (temperature)
        self._node_normalizer = normalization.Normalizer(
            size=1 + common.NodeType.SIZE,
            name='node_normalizer')  # Node comprises temperature and one-hot of node_type
    ...
    def _build_graph(self, inputs, is_training) -> jraph.GraphsTuple:
        ...
        node_type = jax.nn.one_hot(
            inputs['node_type'], one_hot_size).reshape(-1, one_hot_size)

        # node_features is concatenation of temperature and node type
        node_features = jnp.concatenate(
            [inputs['temperature'], node_type], axis=-1)
        node_features_norm = self._node_normalizer.normalize(
            node_features, is_training)

        # Calculate the relative mesh positions
        senders, receivers = common.triangles_to_edges(inputs['cells'])
        relative_mesh_pos = inputs['mesh_pos'][senders] - \
            inputs['mesh_pos'][receivers]

        # edge_features is concatenation of relative mesh positions and norm of
        # these
        edge_features = jnp.concatenate([
            relative_mesh_pos,
            jnp.linalg.norm(relative_mesh_pos, axis=-1, keepdims=True)], axis=-1)
        edge_features_norm = self._edge_normalizer.normalize(
            edge_features, is_training)
